...
st.markdown("""
    <style>
    .hero-banner {
        background: linear-gradient(120deg, #2B1B5E 0%, #3E2C8C 35%, #4D3FBF 65%, #4D96FF 100%);
        border-radius: 22px;
        padding: 46px 46px 40px 46px;
        margin-bottom: 22px;
        box-shadow: 0 10px 30px rgba(77, 61, 191, 0.35);
    }
    .hero-eyebrow {
        color: #C9C3FF;
        font-size: 15px;
...
st.markdown("""
    <div class="hero-banner">
        <p class="hero-eyebrow">🎬 FILM BUSINESS INTELLIGENCE</p>
        <p class="main-title">Movie Revenue&nbsp;<span class="accent">Analysis Dashboard</span></p>
        <p class="sub-title">Interactive Business Intelligence Dashboard for Movie Performance & Revenue Analysis</p>
    </div>
""", unsafe_allow_html=True)

with st.expander("ℹ️ About this project — Business Objective & Problem", expanded=False):
    st.markdown("""
**Business Objective**
The objective of this project is to analyze movie data and help movie producers and production
companies understand what factors make a movie successful. It helps answer questions like:
which movies earn the highest revenue, which genres perform best, whether a higher budget leads
to higher revenue, whether popular movies earn more money, and which movies have the highest
ratings — so companies can make better decisions for future movies.

**Business Problem**
Movie production companies spend a lot of money making movies, but they often don't know which
genre to invest in, how much budget is ideal, which movies become successful, whether popularity
increases revenue, or what factors affect movie ratings. This dashboard analyzes historical data
to answer those questions and support better business decisions.
    """)
...
with tab_overview:
    ...
    fig = px.scatter(filtered_df, x="popularity", y="revenue", color="vote_average", size="budget",
                      hover_name="title", color_continuous_scale="Viridis",
    ...
    highest_budget_movie = filtered_df.loc[filtered_df["budget"].idxmax(), "title"]
    highest_budget_value = filtered_df["budget"].max()

    insights = [
        f"🎬 <b>{highest_revenue_movie}</b> earns the highest revenue in the current selection.",
        f"💵 <b>{highest_budget_movie}</b> has the highest budget in the current selection, at {fmt_money(highest_budget_value)}.",
